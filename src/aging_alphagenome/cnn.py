...
def adamw_update(
    ...
    *,
    ...
    gradient_norm = jnp.sqrt(
        sum(
            jnp.sum(gradient * gradient)
            for gradient in jax.tree_util.tree_leaves(gradients)
        )
    )
    gradient_scale = jnp.minimum(1.0, 1.0 / (gradient_norm + 1e-6))
    gradients = jax.tree_util.tree_map(
        lambda gradient: gradient * gradient_scale, gradients
    )
    ...
    return updated, (first_moment, second_moment, step)
...
def train(args: argparse.Namespace) -> int:
    ...
    def train_step(parameters, optimizer_state, batch_x, batch_y, batch_mask, key):
        ...
        def loss_function(candidate_parameters):
            ...
            return jnp.sum(losses * batch_mask) / jnp.sum(batch_mask)

        loss, gradients = jax.value_and_grad(loss_function)(parameters)
        parameters, optimizer_state = adamw_update(
            jax,
            jnp,
            parameters,
            gradients,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_indices, batch_mask in padded_batches(
